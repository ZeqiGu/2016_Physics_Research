...
for l in f:
    ...
    for ctime in np.arange(ctime1, ctime2, 180):
        ...
        for b in range(int(az-throw),int(az+throw)):          
            ...
            #print(dec,RA)
            if a[1]=="wideAN" or a[1]=="wideAS" or a[1]=="wideB": pix4[int(dec+90)][int(RA)]=sunsidelobe0;
            if a[1]=="day_12h_n" or a[1]=="day_03h_s": pix5[int(dec+90)][int(RA)]=sunsidelobe0;
...
#pix3: depth map; pix4: sidelobe map when sun on west, setting; pix5: when sun on east, rising;           
fig, ax = plt.subplots(2,1)
plt.subplot(211)
plt.imshow(pix4,origin='lower',interpolation='gaussian', cmap=mpl.cm.get_cmap('jet'))
plt.title("Wide")
plt.colorbar(ax=ax[0])
plt.subplot(212)
plt.imshow(pix5,origin='lower',interpolation='gaussian', cmap=mpl.cm.get_cmap('jet'))
plt.title("Day")
